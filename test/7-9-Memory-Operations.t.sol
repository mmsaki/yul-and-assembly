...
contract MemoryTest is Test {
    ...
    function testMemoryPointer() public {
        ...
        assertEq(uint(memAfter) - uint(memBefore), 64);
    }

    function testMemoryPointerV2() public {
        (
            bytes32 memPointerBefore,
            bytes32 msizeBefore,
            bytes32 memPointerAfter,
            bytes32 msizeAfter
        ) = mem.memPointerV2();
        assertEq(
            memPointerBefore,
    ...
    function testAbiEncode() public {
        ...
        assertEq(uint256(memPointerAfter) - uint(memPointerBefore), 96);
    }
    ...
    function testBreakFreeMemoryPointer() public {
        uint256 val = mem.breakFreeMemoryPointer([uint(99)]);
        assertEq(val, 6);
    }
